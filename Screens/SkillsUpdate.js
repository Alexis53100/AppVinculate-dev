...
const SkillsUpdate = ({ navigation, route }) => {
    ...
    const prepararListaSkills = (allSkills, selectedSkills) => {
        let lista = [];

        for (let skill of allSkills) {
            const existsInSelectedSkills = selectedSkills.some(selSkill => selSkill.idSkill === skill.idSkill);
            skill.switch = existsInSelectedSkills;
            lista.push(skill);
        }
    ...
    useEffect(() => {
        ...
        vinculateApi.post('/buscarSkills', { idEmpresa: idEmpresa })
            .then(response => {
                if (response.data.codigo === 1) {
                    return vinculateApi.post('/buscarSkills', { tipo: tipoEmpresa, dato: idEmpresa })
                        .then(response2 => {
                            return [response.data.skils, response2.data.skils];
                        })
            ...
            .then(function ([allSkills, seletedSkills]) {
                prepararListaSkills(allSkills, seletedSkills);
            })
        ...
        if (idSkills.length > 0) {
            ...
            vinculateApi.post('/registrarSkillsEmpresa',
                ...
                .then(function (response) {
                    if (response.data.codigo === 1) {
                        Alert.alert('Aviso Registro Skills', 'Skills actualizados con éxito.');
                    } else {
                        Alert.alert('Advertencia Registro Skills', response.data.texto);
                    }
                    const updateskill = Math.random().toString(36).substring(7);
